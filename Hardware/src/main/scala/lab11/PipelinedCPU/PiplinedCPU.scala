...
class PiplinedCPU(memAddrWidth: Int, memDataWidth: Int) extends Module {
    ...
    // datapath_MEM.io.Mem_Data 改成接 io.DataMem.r.bits.data 的 data

    val DataMemdata = io.DataMem.r.bits.data

    datapath_MEM.io.Mem_Data := MuxLookup(contorller.io.DM_Length, DataMemdata, Seq(
        Byte -> MuxLookup(datapath_MEM.io.Mem_Addr(1,0), DataMemdata(7,0).asUInt,  Seq(
            "b00".U -> DataMemdata(7,0).asUInt,
            "b01".U -> DataMemdata(15,8).asUInt,
            "b10".U -> DataMemdata(23,16).asUInt,
            "b11".U -> DataMemdata(31,24).asUInt
        )),
        Half -> MuxLookup(datapath_MEM.io.Mem_Addr(1), DataMemdata(15,0).asUInt,  Seq(
            "b0".U -> DataMemdata(15,0).asUInt,
            "b1".U -> DataMemdata(31,16).asUInt,
        )),
        Word -> DataMemdata(31,0).asUInt
    ))

    
    // 跟 DMA 一樣做 state machine
    locally {

    val read_Idle :: read_Req_State :: read_Resp_State :: Nil = Enum(3)
    val write_Idle_State :: write_Req_State :: write_Send_State :: write_Resp_State :: Nil = Enum(4)

    val mReadState = RegInit(read_Idle)
    val mWriteState = RegInit(write_Idle_State)
    ...
    io.DataMem.ar.valid       := mReadState === read_Req_State
    // 接到 datapath_MEM.io.Mem_Addr
    io.DataMem.ar.bits.addr   := datapath_MEM.io.Mem_Addr
    io.DataMem.ar.bits.burst  := 0.U
    io.DataMem.ar.bits.len    := 0.U  
    io.DataMem.ar.bits.size   := 2.U  
    ...
    io.DataMem.r.ready := mReadState === read_Resp_State

    // write address channel
    // 用 flag 判別
    io.DataMem.aw.valid       := mWriteState === write_Send_State && !mWriteDataSent
    // 接到 datapath_MEM.io.Mem_Addr
    io.DataMem.aw.bits.addr   := datapath_MEM.io.Mem_Addr
    io.DataMem.aw.bits.burst  := 0.U
    io.DataMem.aw.bits.len    := 0.U  
    io.DataMem.aw.bits.size   := 2.U  
    ...
    io.DataMem.w.valid     := mWriteState === write_Send_State && !mWriteDataSent
    ...
    io.DataMem.b.ready := (mWriteState === write_Resp_State || mWriteState === write_Send_State)


    // read
    when(mReadState === read_Idle) {
        when(contorller.io.DM_Mem_R === 1.U) {
            mReadState := read_Req_State
        }
    }.elsewhen(mReadState === read_Req_State) {
        when(io.DataMem.ar.ready) {
            mReadState := read_Resp_State
        }
    }.elsewhen(mReadState === read_Resp_State) {
    ...
    // write 
    when(mWriteState === write_Idle_State) {
        when(contorller.io.DM_Mem_W === 1.U) {
            mWriteState := write_Req_State
        }
    }.elsewhen(mWriteState === write_Req_State ) {
        when(io.DataMem.aw.ready ) {
            mWriteState :=  write_Send_State
        }
    }.elsewhen( mWriteState === write_Send_State) {
        when( mWriteState === write_Send_State && io.DataMem.w.ready) {
            mWriteState := write_Resp_State
        }
    }.elsewhen(mWriteState === write_Resp_State) {
        when(io.DataMem.b.valid) {
            mWriteState := write_Idle_State
        }
    }

    // Flag for write
    when(mWriteState === write_Send_State) {
